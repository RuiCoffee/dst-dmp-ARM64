...
func (g *Game) initInfo() {
	...
	for _, world := range *g.worlds {
		...
		case "32-bit":
			// DST 服务端仅提供 x86 二进制，ARM 架构下通过 box64 进行二进制翻译运行
			binCmd := utils.WrapX86Binary(fmt.Sprintf("./dontstarve_dedicated_server_nullrenderer -console -cluster %s -shard %s", g.clusterName, world.WorldName))
			startCmd = fmt.Sprintf("cd dst/bin/ && screen -d -h 200 -m -S %s %s", screenName, binCmd)
		case "64-bit":
			binCmd := utils.WrapX86Binary(fmt.Sprintf("./dontstarve_dedicated_server_nullrenderer_x64 -console -cluster %s -shard %s", g.clusterName, world.WorldName))
			startCmd = fmt.Sprintf("cd dst/bin64/ && screen -d -h 200 -m -S %s %s", screenName, binCmd)
		case "luajit":
			// 注意：luajit 模式实际执行的是一个设置 LD_PRELOAD 后再调用真正 ELF 二进制的
			// 包装脚本（见 manual_install.sh 生成的 dontstarve_dedicated_server_nullrenderer_x64_luajit），
			// 而不是可直接被 box64 翻译执行的 ELF 文件，因此这里不对外层调用套用 box64 前缀；
			// ARM 架构下的 box64 转译在该包装脚本内部完成（安装脚本已适配）。
			binCmd := fmt.Sprintf("./dontstarve_dedicated_server_nullrenderer_x64_luajit -console -cluster %s -shard %s", g.clusterName, world.WorldName)
			startCmd = fmt.Sprintf("cd dst/bin64/ && screen -d -h 200 -m -S %s %s", screenName, binCmd)
		default:
			startCmd = "exit 1"
		}
