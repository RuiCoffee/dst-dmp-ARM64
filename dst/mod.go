...
func (g *Game) generateModDownloadCmd(id int) string {
	args := fmt.Sprintf("+force_install_dir %s/%s/mods/ugc/%s +login anonymous +workshop_download_item 322330 %d +quit", db.CurrentDir, utils.DmpFiles, g.clusterName, id)
	// ARM 架构下会自动改为通过 box64 调用 steamcmd/linux32/steamcmd 二进制
	return utils.SteamCmdCmd("steamcmd", args)
}
